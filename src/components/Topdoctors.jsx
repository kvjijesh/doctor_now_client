...
const Topdoctors = () => {
    ...
    return (
        <>
            <Box container flexDirection={'column'} mb={10}>
                    ...
                    {doctors.map((item, i) => {
                        return (
                            <Paper key={i} display={'flex'} flexDirection={'column'} sx={{ width: 250, m:3 ,boxShadow:2}}>
                                <Grid display={'flex'} justifyContent={'center'} mt={2}>
                                {item.image?(<img style={{ width: 100, height: 100, borderRadius: '50%' }} src={`http://localhost:8000/images/${item.image}`} alt="" />):(<img style={{ width: 100, height: 100, borderRadius: '50%' }} src={`https://upload.wikimedia.org/wikipedia/commons/b/bc/Unknown_person.jpg`} alt="" />)}
                                </Grid>
